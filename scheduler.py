        ...
        try:
            with open(COUNTS_FILE, "r", encoding="utf-8") as f:
                ...
                if data.get("date") == today_str:
                    ...
                    if "successful_posts_today" not in data:
                        data["successful_posts_today"] = 0
                    return data
        except Exception:
            pass
...
def check_and_run_poster():
    ...
    # التحقق من سقف النشر اليومي (100 منشور ناجح كحد أقصى)
    successful_today = counts.get("successful_posts_today", 0)
    if successful_today >= 100:
        print(f"[!] تنبيه: تم الوصول للحد الأقصى اليومي للمنشورات الناجحة ({successful_today}/100). لن يتم النشر الآن.")
        return
        
    print(f"[*] تشغيل منشور عادي... (العداد الحالي للمنشورات الناجحة اليوم: {successful_today}/100)")
    ...
    if target_type:
        print(f"[+] تم تحديد منشور مستهدف لتلبية الحصة اليومية: [{target_type}].")
    
    try:
        # محاولة النشر والتحقق من النتيجة (True = نجاح، False = فشل)
        success = run_poster(override_type=target_type)
        
        if success:
            # زيادة العداد الإجمالي للمنشورات الناجحة اليوم
            counts["successful_posts_today"] = counts.get("successful_posts_today", 0) + 1
            
            # زيادة العدادات التفصيلية للنوع المستهدف
            if target_type == "coin_analysis":
                counts["last_coin_analysis_hour"] = hour
            elif target_type == "market_status":
            ...
            print(f"[+] اكتملت عملية النشر بنجاح! العداد اليومي الحالي: {counts['successful_posts_today']}/100")
        else:
            print("[-] فشلت عملية النشر على المنصة. لن يتم احتساب المنشور ضمن العداد اليومي.")
